...
// ─── Interior Walls ─────────────────────────────────────────────────────────
// Keep it simple: just the main room dividers with doorway gaps.
// Both Entry/WD and Bathroom are in the SETBACK zone (north wall at NOTCH_Z).
// Only Bed1 and Bed2 extend to N_WALL.
function InteriorWalls() {
  const W = MAT.interior
  const doorGap = 3.0 // standard doorway width

  // Useful interior face refs
  const westFace = W_WALL + EXT / 2   // -12.25
  const eastFace = E_WALL - EXT / 2   // 12.25
  const notchFace = NOTCH_Z + EXT / 2 // north int face of setback rooms

  return (
    <group>
      {/* ═══ 1. E-W PARTITION — separates north rooms from living ═══ */}
      {/* Segment A: west wall to hallway gap */}
      <Wall position={[(westFace + HALL_OPEN_L) / 2, WALL_H / 2, PARTITION_Z]}
        size={[HALL_OPEN_L - westFace, WALL_H, INT]} color={W} castShadow={false} />
      {/* Segment B: hallway gap to east wall */}
      <Wall position={[(HALL_OPEN_R + eastFace) / 2, WALL_H / 2, PARTITION_Z]}
        size={[eastFace - HALL_OPEN_R, WALL_H, INT]} color={W} castShadow={false} />

      {/* ═══ 2. N-S: Entry/WD | Bathroom (X = -5.13) ═══ */}
      {/* Runs from NOTCH_Z to PARTITION_Z (both rooms are in setback zone) */}
      <Wall position={[X_BED2_BATH, WALL_H / 2, (NOTCH_Z + PARTITION_Z) / 2]}
        size={[INT, WALL_H, Math.abs(PARTITION_Z - NOTCH_Z)]} color={W} castShadow={false} />

      {/* ═══ 3. N-S: Bath | Bed1 (X = 0.535) — the notch boundary ═══ */}
      {/* Runs from NOTCH_Z to PARTITION_Z only (exterior handles N_WALL to NOTCH_Z) */}
      {/* Door gap near south end for hallway→Bed1 access */}
      <Wall position={[X_BATH_EAST, WALL_H / 2, (NOTCH_Z + (PARTITION_Z - doorGap)) / 2]}
        size={[INT, WALL_H, (PARTITION_Z - doorGap) - NOTCH_Z]} color={W} castShadow={false} />

      {/* ═══ 4. N-S: Bed1 | Bed2 (X = 12.5, at main east wall line) ═══ */}
      {/* Runs from N_WALL to STEP_Z. Door gap near south end. */}
      <Wall position={[E_WALL, WALL_H / 2, (N_WALL + (STEP_Z - doorGap)) / 2]}
        size={[INT, WALL_H, (STEP_Z - doorGap) - N_WALL]} color={W} castShadow={false} />

      {/* ═══ 5. E-W: Bathroom south wall ═══ */}
      {/* From Entry/Bath wall to Bath/Bed1 wall, with door gap on west side */}
      <Wall position={[(X_BED2_BATH + doorGap + X_BATH_EAST) / 2, WALL_H / 2, BATH_S_Z]}
        size={[X_BATH_EAST - X_BED2_BATH - doorGap, WALL_H, INT]} color={W} castShadow={false} />

      {/* ═══ 6. E-W: Bed1 closet wall (2' deep at south end of Bed1) ═══ */}
      <Wall position={[(X_BATH_EAST + E_WALL) / 2, WALL_H / 2, BED1_CLOSET_Z]}
        size={[E_WALL - X_BATH_EAST, WALL_H, INT]} color={W} castShadow={false} />
    </group>
  )
}
